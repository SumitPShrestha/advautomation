...
public class SambaConnection {
    ...
    private static String DOMAIN = "d2hawkeye";
    private static String USERNAME = "seleniumrun";
    private static String PASSWORD = "nepal888!";
    private static String SMB = "smb:";

    private SmbFile remoteFile;

    public SambaConnection(String nameWithWildcard, String folderLocation) throws  FileNotFoundException {
    ...
    * */
    public SmbFile getFileInDirForNameWithWildcard(SmbFile fileDir, final String nameWithWildcard) throws FileNotFoundException{
        try {
            ...
            logger.error("Could not find file with file name " + nameWithWildcard + " in location " + fileDir.getCanonicalPath());
        }
        throw new FileNotFoundException("There was an Error trying to make connection to " + fileDir.getCanonicalPath());
        }
